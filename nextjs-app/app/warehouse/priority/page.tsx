...
export default async function WarehousePriority() {
  const { data: rows } = await supabase
    ...
    .select(`
      order_id, late_probability, predicted_late,
      orders(order_total, order_datetime, payment_method,
        customers(full_name)),
      shipments:order_id(carrier, shipping_method, distance_band, promised_days, actual_days, late_delivery)
    `)
    .order("late_probability", { ascending: false })
    .limit(50);

  // Supabase joins with same-name FK can be tricky — flatten manually
  // Use a simpler approach: two queries
